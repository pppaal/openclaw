...
## 🖥️ 관리자 대시보드 (가게 여러 개 관리)
PowerShell·메모장 없이 **브라우저에서 클릭으로** 가게를 추가/수정해요. 여러 가게(`shops/` 폴더)를 한 번에 관리.
```bash
npm run admin     # http://localhost:4000
```
- 가게 목록 보기 / 새 가게 추가 / 가게 정보·메뉴·FAQ 수정
- 수정하면 봇 답변에 바로 반영
...
npm run report
```
손님이 뭘 물어봤는지 + 봇이 답 못 해서 전화로 넘긴 질문을 모아줘요 → 그 답을 FAQ에 추가하면 봇이 더 똑똑해짐.
```
나 > 커트 얼마예요?
